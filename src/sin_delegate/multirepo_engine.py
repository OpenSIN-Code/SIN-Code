...
class MultiRepoDelegator:
    def __init__(
        self, mrp: MultiRepoPlan, ledger: Ledger | None = None, max_parallel: int = 4
    ) -> None:
        mrp.validate()
        self.mrp = mrp
        self.plan = mrp.plan
        ...
        self.wtms = {
            name: WorktreeManager(ref.path, ref.base_branch) for name, ref in mrp.repos.items()
        }
    ...
    async def run(self) -> RunResult:
        started = time.time()
        self.ledger.register_run(self.plan.id, self.plan.goal, self._plan_json())
        scheduler = Scheduler(
            self.plan, self.ledger, self._execute_task, max_parallel=self.max_parallel
        )
        outcomes = await scheduler.run()

        all_verified = all(
            o.state in (TaskState.DONE, TaskState.SKIPPED) for o in outcomes.values()
        )
        if all_verified and self.merger.units:
            try:
                ...
                for unit in self.merger.units:
                    ...
                    self.ledger.emit(
                        self.plan.id, unit.task_id, "state:escalated", {"error": str(e)}
                    )
        elif self.merger.units:
            self.ledger.emit(
                self.plan.id,
                "*",
                "merge:phase2_withheld",
                {"reason": "not all tasks verified", "staged": len(self.merger.units)},
            )

        return RunResult(self.plan.id, self.plan.goal, outcomes, started, time.time())
    ...
    async def _execute_task(self, task: Task) -> TaskOutcome:
        ...
        res = await runner.run(enriched, str(wt.path), timeout=task.budget.max_seconds)
        self.ledger.emit(
            self.plan.id,
            task.id,
            "agent:finished",
            {"repo": repo_name, "exit": res.exit_code, "tail": res.output[-2000:]},
        )
        if not res.ok:
        ...
        if not wt.commit_all(f"sin-delegate: {task.title} [{task.id}]"):
            outcome.state = TaskState.FAILED
        ...
        self.ledger.emit(
            self.plan.id, task.id, "verdict", {"passed": verdict.passed, "gates": verdict.gates}
        )

        if not verdict.passed:
            memory.remember_pitfall(self.plan.goal, task.title, verdict.summary)
            if task.risk == Risk.HIGH:
                esc = self.broker.raise_escalation(
                    ...
                    summary=f"HIGH-risk task failed gates: {verdict.summary}",
                    evidence={
                        "gates": verdict.gates,
    ...
    def _enrich(self, task: Task) -> Task:
        ...
        if self._pitfalls:
            extra.append("Bekannte Pitfalls (vermeiden):\n- " + "\n- ".join(self._pitfalls))
        if task.id in self._retry_context:
            extra.append(self._retry_context[task.id])
        return replace(task, instructions=task.instructions + "\n\n" + "\n\n".join(extra))
        ...
        return json.dumps(
            {
                ...
                "multi_repo": True,
                "repos": {
                    n: {"path": r.path, "base_branch": r.base_branch}
