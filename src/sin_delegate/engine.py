...
class Delegator:
    def __init__(
        ...
        self.wtm = None if dry_run else WorktreeManager(plan.repo, plan.base_branch)
    ...
    async def run(self) -> RunResult:
        ...
        result = RunResult(self.plan.id, self.plan.goal, outcomes, started, time.time())
        if result.ok:
    ...
    async def _execute_task(self, task: Task) -> TaskOutcome:
        if self.dry_run:
            ...
            return TaskOutcome(task.id, TaskState.DONE, worktree="(dry-run)")
        ...
        try:
            ...
            self.ledger.emit(
                ...
                task.id,
                "agent:finished",
                {"exit": res.exit_code, "tail": res.output[-2000:]},
            )
            if not res.ok:
            ...
            if not verdict.passed:
                ...
                if task.risk == Risk.HIGH:
                    ...
                    outcome.error = f"HIGH-risk task failed gates: {verdict.summary}"
                else:
            ...
            try:
                ...
                self.ledger.emit(self.plan.id, task.id, "merged", {"snapshot": snapshot})
    ...
    def _verdict_feedback(verdict) -> str:
        failed = {n: r["detail"] for n, r in verdict.gates.items() if not r["ok"]}
...
def delegate(
    ...
    plan = Plan(
        goal=goal, tasks=tuple(t.finalize() for t in tasks), repo=repo, base_branch=base_branch
    )
    return Delegator(plan, max_parallel=max_parallel, dry_run=dry_run).run_sync()
