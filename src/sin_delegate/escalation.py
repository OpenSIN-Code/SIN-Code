...
def _options_for(kind: EscalationKind, branch: str) -> list:
    ...
    common_abort = Option(
        "abort",
        ActionType.ABORT_PLAN,
        "Gesamten Plan abbrechen",
    ...
    if kind == EscalationKind.GATE_FAILURE:
        return [
            Option(
                "retry",
                ActionType.RETRY_WITH_GUIDANCE,
    ...
    if kind == EscalationKind.BUDGET_EXHAUSTED:
        return [
            Option(
                ...
                "Der Task erhält ein frisches Lease außerhalb des ursprünglichen Global-Budgets.",
...
class EscalationBroker:
    ...
    def raise_escalation(
        ...
        self.ledger.emit(plan_id, task_id, "escalation:raised", esc.to_dict())
        return esc
    ...
    def resolve(
        ...
        if esc is None:
            return {
                "ok": False,
                "error": f"escalation {escalation_id} not open (unknown or already resolved)",
            }
        option = next((o for o in esc["options"] if o["id"] == option_id), None)
        if option is None:
            valid = [o["id"] for o in esc["options"]]
            return {"ok": False, "error": f"unknown option {option_id!r}; valid: {valid}"}
        if option["requires_input"] and not user_input.strip():
            return {
                "ok": False,
                "error": f"option {option_id!r} requires input (e.g. guidance for the retry)",
            }
        try:
            action = ActionType(option["action"]).value
        except ValueError:
            return {"ok": False, "error": f"escalation has invalid action {option['action']!r}"}
        self.ledger.emit(
            plan_id,
            esc["task_id"],
            "escalation:resolved",
    ...
    def pending_resolutions(self, plan_id: str) -> list:
        ...
        return [r for r in resolutions if r["escalation_id"] not in applied]

    def mark_applied(self, plan_id: str, task_id: str, escalation_id: str) -> None:
        self.ledger.emit(plan_id, task_id, "escalation:applied", {"escalation_id": escalation_id})
