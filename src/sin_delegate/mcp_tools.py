...
_PLAN_SCHEMA = {
    ...
    "required": ["plan"],
    "properties": {
        "plan": {"type": "object", "description": "Plan: {goal, base_branch?, tasks:[{...}]}"},
...
async def _tool_delegate(args: dict) -> dict:
    ...
    dele = Delegator(plan, max_parallel=parallel, dry_run=bool(args.get("dry_run", False)))
...
async def _tool_status(args: dict) -> dict:
    ...
    return {"plan_id": pid, "states": {k: v.value for k, v in states.items()}}
...
async def _tool_escalations(args: dict) -> dict:
    ...
    if isinstance(pid, dict):
        return pid
    return {"plan_id": pid, "escalations": EscalationBroker().open_escalations(pid)}
