...
class BudgetGovernor:
    ...
    _allocs: dict[str, Allocation] = field(init=False, default_factory=dict)
    ...
    def _estimate(self, task: Task) -> float:
        ...
        est = self.analytics.expected_seconds(task_class_of(task), default=task.budget.max_seconds)
        return min(est * self.safety_factor, task.budget.max_seconds)

    def _seed(self) -> None:
        estimates = {t.id: max(self._estimate(t), self.min_grant) for t in self.plan.tasks}
        total = sum(estimates.values())
        scale = min(1.0, self.global_seconds / total) if total else 1.0
        for tid, est in estimates.items():
            self._allocs[tid] = Allocation(tid, granted=max(est * scale, self.min_grant))
        self._pool = max(0.0, self.global_seconds - sum(a.granted for a in self._allocs.values()))
    ...
    def _pressure(self) -> float:
        ...
        return min(1.0, self.remaining_global() / max(self.global_seconds, 1.0) * 2.0)
    ...
    async def lease(self, task_id: str) -> float:
        async with self._lock:
            ...
            grant = max(grant, min(self.min_grant, self.remaining_global()))
    ...
    async def request_extension(self, task_id: str, seconds: float) -> float:
        async with self._lock:
            if self._pool <= 0:
                return 0.0
            max_prio = max(self.priority.values()) or 1
            weight = self.priority.get(task_id, 1) / max_prio
            grant = min(seconds * weight, self._pool, self.remaining_global())
            if grant < 5.0:
                return 0.0
            self._pool -= grant
    ...
    def snapshot(self) -> dict:
        return {
            ...
            "allocations": {
                tid: {"granted": round(a.granted, 1), "extended": round(a.extended, 1)}
