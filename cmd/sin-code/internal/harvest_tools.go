...
// Purpose: harvest — URL fetching with caching, structure extraction, and
// change detection. Built-in Go implementation using net/http with local disk
// cache wrapped in a circuitbreaker transport so a slow/unresponsive upstream
// cannot pin the agent loop indefinitely (#72).
// Split from analysis_tools.go to satisfy the shrink recommendation in issue #426.
// sin-debt: shrink, upgrade: when a second harvest-related function is needed, merge into a shared file
package internal
