...
//
// Deterministic compaction engine — SHA-256 dedupe +
...
// normalizedSet is the worker behind Plan(strategy=deterministic).
...
// normalize is the per-target entry loader the worker above
// relies on. The actual readers live in types.go (lessons,
