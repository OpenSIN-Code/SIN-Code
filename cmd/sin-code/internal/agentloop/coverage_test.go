...
// ---------------------------------------------------------------------------
// epic_coordinator.go coverage
// ---------------------------------------------------------------------------

func TestEpicCoordinator_NilLoader_LoadEpic(t *testing.T) {
	c := NewEpicCoordinator()
	// Default loader is nilLoader which returns ErrEpicNotFound
	_, err := c.LoadEpic(42)
	if err != ErrEpicNotFound {
		t.Fatalf("expected ErrEpicNotFound from nilLoader, got: %v", err)
	}
}

func TestEpicCoordinator_NilLoader_LoadDependencies(t *testing.T) {
	c := NewEpicCoordinator()
	deps, err := c.Dependencies(42)
	if err != nil {
		t.Fatalf("unexpected error from nilLoader LoadDependencies: %v", err)
	}
	if deps != nil {
		t.Errorf("expected nil deps from nilLoader, got %v", deps)
	}
}

func TestEpicCoordinator_SetLoader_NilCoordinator(t *testing.T) {
	var c *EpicCoordinator
	c.SetLoader(mockLoader{}) // should not panic
}

func TestEpicCoordinator_LoadEpic_NilEpicFromLoader(t *testing.T) {
	c := NewEpicCoordinator()
	c.SetLoader(mockLoader{
		epics: map[int]*Epic{
			10: nil, // loader returns nil epic
		},
	})
	_, err := c.LoadEpic(10)
	if err != ErrEpicNotFound {
		t.Fatalf("expected ErrEpicNotFound for nil epic, got: %v", err)
	}
}

func TestEpicCoordinator_MarkComplete_AlreadyComplete(t *testing.T) {
	c := NewEpicCoordinator()
	c.SetLoader(mockLoader{
		epics: map[int]*Epic{
			100: {IssueNumber: 100, Title: "Epic", SubIssues: []int{101, 102}, Completed: []int{101}},
		},
	})
	epic, _ := c.LoadEpic(100)
	// Mark 101 complete again — should be idempotent
	if err := c.MarkComplete(101); err != nil {
	...
	if count != 1 {
		t.Errorf("expected 101 to appear once, got %d", count)
	}
}

func TestEpicCoordinator_MarkComplete_NotSubIssue(t *testing.T) {
	c := NewEpicCoordinator()
	c.SetLoader(mockLoader{
		epics: map[int]*Epic{
			100: {IssueNumber: 100, Title: "Epic", SubIssues: []int{101, 102}, Completed: nil},
		},
	})
	epic, _ := c.LoadEpic(100)
	// 999 is not a sub-issue of epic 100 — should not be added to Completed
	_ = c.MarkComplete(999)
	for _, num := range epic.Completed {
		if num == 999 {
			t.Error("non-sub-issue should not be added to epic.Completed")
		}
	}
}

func TestEpicCoordinator_MarkComplete_NilCoordinator(t *testing.T) {
	var c *EpicCoordinator
	err := c.MarkComplete(1)
	if err != ErrEpicNotFound {
		t.Errorf("nil MarkComplete should return ErrEpicNotFound, got %v", err)
	}
}

func TestEpicCoordinator_Dependencies_NilCoordinator(t *testing.T) {
	var c *EpicCoordinator
	_, err := c.Dependencies(1)
	if err != ErrEpicNotFound {
		t.Errorf("nil Dependencies should return ErrEpicNotFound, got %v", err)
	}
}

func TestEpicCoordinator_Dependencies_ErrorFromLoader(t *testing.T) {
	c := NewEpicCoordinator()
	c.SetLoader(errorLoader{})
	_, err := c.Dependencies(42)
	if err == nil {
		t.Fatal("expected error from errorLoader")
	}
}

func TestEpicCoordinator_LoadEpic_ErrorFromLoader(t *testing.T) {
	c := NewEpicCoordinator()
	c.SetLoader(errorLoader{})
	_, err := c.LoadEpic(42)
	if err == nil {
...
func TestEpicCoordinator_Progress_NoSubIssues(t *testing.T) {
	c := NewEpicCoordinator()
	epic := &Epic{IssueNumber: 100, Title: "Empty", SubIssues: nil}
	if got := c.Progress(epic); got != 1.0 {
		t.Errorf("expected 1.0 for no sub-issues, got %.2f", got)
	}
}

func TestEpicCoordinator_Progress_NilEpic(t *testing.T) {
	c := NewEpicCoordinator()
	if got := c.Progress(nil); got != 0 {
		t.Errorf("nil Progress should return 0, got %.2f", got)
	}
}

func TestEpicCoordinator_Summary_NilCoordinator(t *testing.T) {
	var c *EpicCoordinator
	if got := c.Summary(nil); got != "epic: <unknown>" {
		t.Errorf("nil Summary should return 'epic: <unknown>', got %q", got)
	}
}

func TestEpicCoordinator_Summary_AllComplete(t *testing.T) {
	c := NewEpicCoordinator()
	c.SetLoader(mockLoader{
		epics: map[int]*Epic{
			100: {IssueNumber: 100, Title: "Done", SubIssues: []int{101, 102}, Completed: nil},
		},
	})
	epic, _ := c.LoadEpic(100)
	c.MarkComplete(101)
	...
	if !strings.Contains(summary, "all complete") {
		t.Errorf("summary should say 'all complete', got: %s", summary)
	}
}

func TestEpicCoordinator_Summary_OverFull(t *testing.T) {
	c := NewEpicCoordinator()
	c.SetLoader(mockLoader{
		epics: map[int]*Epic{
			100: {IssueNumber: 100, Title: "Epic", SubIssues: []int{101}, Completed: nil},
		},
	})
	epic, _ := c.LoadEpic(100)
	// Mark complete multiple times to push progress over 100%
	c.MarkComplete(101)
	summary := c.Summary(epic)
	if summary == "" {
...
func TestEpicCoordinator_NextIssue_NilCoordinator(t *testing.T) {
	var c *EpicCoordinator
	_, err := c.NextIssue(&Epic{SubIssues: []int{1}})
	if err != ErrNoRemainingIssues {
		t.Errorf("nil NextIssue should return ErrNoRemainingIssues, got %v", err)
	}
}

// errorLoader returns errors for all methods.
type errorLoader struct{}

func (errorLoader) LoadEpic(issueNumber int) (*Epic, error) {
	return nil, errors.New("loader error")
}

func (errorLoader) LoadDependencies(issueNumber int) ([]int, error) {
	return nil, errors.New("loader error")
}
