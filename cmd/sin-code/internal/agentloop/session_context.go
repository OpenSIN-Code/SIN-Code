...
// memory has no current dependency on agentloop but that contract is
...
// unless the caller overrides it. Init-order safe: secretPatterns +
...
// loop calls its Invoke at session start when any opt-in flag is set.
//
// A zero-value ContextInjector is safe; Enabled() reports false.
// Callers set fields directly (no constructor pattern), e.g.:
//
//	inj := agentloop.ContextInjector{
//	    Lessons: ls, Memory: mem, Goals: q,
//	    Workspace: ws, TopK: 5,
//	    InjectLessons: true,
//	    InjectMemory:  true,
//	    InjectGoals:   false,
...
type ContextInjector struct {
	...
	// nil → DefaultRedactor(). The constructor below makes the default
	// explicit so a test that wants raw content can pass
	// passingRedactor{} rather than nil.
...
//
// Result is byte-stable per (config, sources, prompt) — see
// session_context_test.go for the golden invariants:
//
//   - marker ordering: START → sections → END
//   - section headers are exactly # Relevant lessons | # Relevant
//     memory | # Pending autonomous goals
//   - lessons sorted by occurrences DESC then last_seen DESC
//   - memory sorted by score DESC (the score the underlying Prime
//     string built with — Prime already emits its own internal order)
//   - goals sorted by priority DESC then id ASC
...
// buildLessonsSection runs lessons.Store.QueryTopK when available and
// falls back to lessons.Store.Query when the index returns nothing
// (the legacy path is also wired for backward compat — QueryTopK
...
// buildMemorySection calls Memory.Prime with the same query the
// agent is about to receive, then re-renders so the redactor runs.
// Prime already returns a formatted block; we strip / re-add framing
// to keep the section header byte-stable across Prime version bumps.
func (c *ContextInjector) buildMemorySection(ctx context.Context, prompt string, topK int) (string, error) {
	...
	// Strip Memory.Prime's own header so our section header is the only
	// "##" / "#" line in the rendered block — keeps the manifest
	// byte-stable.
