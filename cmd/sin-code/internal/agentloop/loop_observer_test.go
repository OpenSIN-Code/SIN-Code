...
// the surrounding Loop wiring. Wiring integration lives in
// loop_observer_wiring_test.go (smoke) plus the loop.Run dispatch
// path in loop.go.
package agentloop
...
// reordered-but-equal calls do not escape the detector.
func TestObserverArgsSignature(t *testing.T) {
	...
	// Now a call with a different body must NOT trip the same key.
	d2 := NewLoopDetector(10, 2, 2)
	_ = d2.Observe(mkObsTC("write", "body", "v1"), "")
	_ = d2.Observe(mkObsTC("write", "body", "v2"), "")
	_ = d2.Observe(mkObsTC("write", "body", "v1"), "")
	_ = d2.Observe(mkObsTC("write", "body", "v2"), "")
	if d2.LastTrip() != nil {
		t.Fatalf("args-distinct cycle should not trip")
	}
}
