...
// ──────────────────────────────────────────────────────────────────────────
// Context Compaction Modes (issue: compaction-modes, first PR) coverage.
// Test cases below exercise Compact2 with mock summarizers and verify
// evidence preservation across deterministic / llm / hybrid modes.
// ──────────────────────────────────────────────────────────────────────────

// stubSummarizer returns a fixed summary prefixed with the requested tag.
// Returns the tag verbatim so tests can assert round-trip integrity.
func stubSummarizer(tag string) SummarizerFunc {
	return func(ctx context.Context, msgs []session.Message) (string, error) {
		return tag + "(" + messageFingerprint(msgs) + ")", nil
	}
}

// messageFingerprint builds a stable fingerprint of a slice purely for
// summarizer-stub spy purposes. It does NOT need to be unique — it just
// needs to be deterministic so the stub returns a stable string.
func messageFingerprint(msgs []session.Message) string {
...
func TestCompact2_Deterministic_EvidencePreserved(t *testing.T) {
	...
	// 30 messages so the recent-turns retain window does not swallow
	// everything; we want non-flagged messages to actually drop.
	...
	// Evidence flags that should survive.
...
func TestCompact2_Hybrid_EvidencePlusSummary(t *testing.T) {
	...
	// Evidence must remain reachable in the post-summary tail.
	for _, idx := range []int{0, 1, 7} { // system, goal, VERIFICATION PASSED
		if !containsMessage(r.Kept, msgs[idx]) {
...
func TestCompact2_LegacyStrategy_Unchanged(t *testing.T) {
	// When input.Mode is empty, Compact2 should fall back to the legacy
	// explicit-strategy path byte-for-byte for the issue #278 callers.
	c := NewCompactor(stubSummarizer("LLM"))
	msgs := makeTestMessages(20)
	r, err := c.Compact2(context.Background(), CompactInput{
		Messages:  msgs,
...
// messagesEqual compares two session.Message values structurally. The
// embedded json.RawMessage slice does not support !=; compare bytes.
...
func TestCompact2_RecentTurnsConfigurable(t *testing.T) {
	...
	c.Configure(CompactorConfig{
		Mode:             ContextCompactionDeterministic,
		RecentTurns:      2,
		PreserveEvidence: false, // pure recent-turns retain
		MaxTokens:        8000,
	})
	...
	// With RecentTurns=2, the tail should be at most 4 messages
	// (rule 3: tailStart = max(0, len-2*2) = 26-29), plus system + goal.
	// Expected = system(0) + goal(1) + tail(4) = 6.
...
func TestIdentifyEvidence_FindsMarkers(t *testing.T) {
	...
	idx := identifyEvidence(msgs)
	wantSet := map[int]bool{2: true, 4: true}
	if len(idx) != len(wantSet) {
		t.Errorf("identifyEvidence set size: got %d want %d (idx=%v)", len(idx), len(wantSet), idx)
	}
	for k := range wantSet {
		if !idx[k] {
			t.Errorf("identifyEvidence missing index %d (idx=%v)", k, idx)
		}
	}
}
