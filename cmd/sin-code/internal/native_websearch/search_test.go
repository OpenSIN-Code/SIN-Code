...
// (issue #381). Tests cover:
//   1. HTML parsing of a DuckDuckGo-shaped fixture (TestSearchWithMockedHTML)
//   2. Rate-limiter behaviour under burst (TestSearchRateLimit)
//   3. Cache hit avoidance of the network round trip (TestSearchCache)
//   4. Robots.txt Disallow honoured at the search path (TestSearchRobots)
//   5. Empty-query typed error (TestSearchEmptyQuery)
//   6. Race-cleanliness under -race for the LRU + limiter together
//
// All network surfaces are mocked with httptest.Server so the suite
// runs in milliseconds and never reaches the public internet.
...
// fixture at any path, and a permissive robots.txt at the well-known
// /robots.txt location. Tests that need a forbidden robots.txt set
// robotsBody manually via Server config.
