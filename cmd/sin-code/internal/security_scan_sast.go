// SPDX-License-Identifier: MIT // sin-debt: shrink, acceptable: one exported subcommand constructor per file matches project convention
// Purpose: `sin-code security scan sast` — vendored SAST scanner integration.
// Locates the sin-sast binary (or builds it from the vendored
// SIN-Code-SAST-Tool module), runs it with JSON output, parses the result,
// and prints findings in a friendly format.
...
// sin-debt: yagni, acceptable: required subcommand entry point for `security scan sast`
