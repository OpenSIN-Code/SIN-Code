...
var externalServers = []externalServer{
	...
	{name: "browser", namespace: "browser__*", short: "Browser automation (106 tools)", description: "SIN-Browser-Tools MCP — 106 Playwright-based browser automation tools: navigation, click/type/fill, screenshots, PDF, tab/session management, cookie/storage, diagnostics, Shadow DOM, OOPIF, SPA wait, network mocking, macOS Spaces, screen recording.", example: "browser__navigate https://example.com", tags: []string{"external", "browser", "automation", "playwright"}},
	...
	{name: "marketplace", namespace: "marketplace__*", short: "Skill marketplace", description: "Marketplace MCP server bundled in the SIN-Code Python companion package.", example: "marketplace__search skill", tags: []string{"bundled", "skills"}},
	{name: "mcpbuilder", namespace: "mcpbuilder__*", short: "MCP server builder", description: "MCP Server Builder bundled in the SIN-Code Python companion package.", example: "mcpbuilder__scaffold --name my-tool", tags: []string{"bundled", "mcp"}},
	...
	{name: "notion", namespace: "notion__*", short: "Notion full access", description: "vibe-notion MCP bridge — 17 tools: 10 read (workspaces, search, pages, databases, blocks, comments, users) + 6 write (create/update/archive pages, append/delete blocks, comments) + 1 raw CLI. Act-as-user via token_v2 or bot via NOTION_TOKEN.", example: "notion__notion_read_workspaces", tags: []string{"external", "productivity", "notion"}},
}
