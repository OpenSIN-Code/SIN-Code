...
var chatTools = []chatTool{
	...
	{name: "spawn_subgoal", short: "Sync sub-goal", description: "Enqueue a verified sub-goal and BLOCK until it terminates, fails, or times out. Each sub-goal still passes the verify gate (M3).", example: `{\"title\": \"fix bug\", \"description\": \"Refactor auth.go to close the race on session ID.\", \"max_turns\": \"20\", \"timeout\": \"2m\"}`, tags: []string{"destructive", "autonomy", "sub-goal"}, destructive: true},
}
