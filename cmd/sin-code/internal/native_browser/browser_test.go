...
//
// Build tag: nethttp isolates httptest-based tests so the rest of the
// package compiles for anyone who has stripped the stdlib test server —
// nethttp is opt-in.
...
func TestBrowserSnapshot(t *testing.T) {
	...
	// Snapshot from memory: no additional driver.Load calls.
	drv.mu.Lock()
...
// when the selector never appears (here: a deadline too tight for any
// network round trip).
