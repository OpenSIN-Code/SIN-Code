...
func (s *BrowserSession) WaitFor(selector string) error {
	...
	for {
		...
		case <-ctx.Done():
			if selector == "" {
				return fmt.Errorf("native_browser: waitfor timed out: %w", ctx.Err())
			}
...
func selectorPresent(html, selector string) bool {
	...
	// Strip surrounding whitespace from a CSS-like selector.
	sel := strings.TrimSpace(selector)
	return strings.Contains(html, sel)
}

// nopCloser is the io.Closer returned by Drivers whose Load/Perform do not
// stream a body. Keeps the Driver interface symmetric.
