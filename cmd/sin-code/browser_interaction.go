...
func toolBrowserScreenshot(ctx context.Context, selector, qualityStr string) (string, error) {
	if browserSession == nil { return "", fmt.Errorf("sin_browser_screenshot: no active browser session") }
	quality := 80
	if qualityStr != "" { fmt.Sscanf(qualityStr, "%d", &quality) }
	var buf []byte
	var err error
	if selector != "" {
	...
	if err != nil { return "", fmt.Errorf("sin_browser_screenshot: %w", err) }
...
func toolBrowserClick(ctx context.Context, selector string) (string, error) {
	if browserSession == nil { return "", fmt.Errorf("sin_browser_click: no active browser session") }
	...
	if err != nil { return "", fmt.Errorf("sin_browser_click: %w", err) }
...
func toolBrowserType(ctx context.Context, selector, text, submitStr string) (string, error) {
	...
	if submit { actions = append(actions, chromedp.SendKeys(selector, "\n", chromedp.ByQuery)) }
	err := chromedp.Run(browserSession.cdpCtx, actions...)
	if err != nil { return "", fmt.Errorf("sin_browser_type: %w", err) }
	r := "typed " + text + " into " + selector
	if submit { r += " and submitted" }
	return r, nil
}

func toolBrowserEval(ctx context.Context, expr string) (string, error) {
	if browserSession == nil { return "", fmt.Errorf("no active browser session") }
	if expr == "" { return "", fmt.Errorf("expression required") }
	var result any
	err := chromedp.Run(browserSession.cdpCtx, chromedp.Evaluate(expr, &result))
	if err != nil { return "", fmt.Errorf("sin_browser_eval: %w", err) }
...
func toolBrowserWait(ctx context.Context, selector, timeoutStr string) (string, error) {
	...
	if err != nil { return fmt.Sprintf("wait failed: %v", err), nil }
...
func registerBrowserInteractionSpecs() []agentloopToolSpecAlias {
	...
	o := func(p map[string]any, r ...string) map[string]any { return map[string]any{"type": "object", "properties": p, "required": r} }
...
func dispatchBrowserInteraction(ctx context.Context, name string, args map[string]any) (string, bool, error) {
	switch name {
	case "sin_browser_screenshot": s,_ := args["selector"].(string); q,_ := args["quality"].(string); out,err := toolBrowserScreenshot(ctx,s,q); return out,true,err
	case "sin_browser_click": s,_ := args["selector"].(string); out,err := toolBrowserClick(ctx,s); return out,true,err
	case "sin_browser_type": s,_ := args["selector"].(string); t,_ := args["text"].(string); sub,_ := args["submit"].(string); out,err := toolBrowserType(ctx,s,t,sub); return out,true,err
	case "sin_browser_eval": e,_ := args["expr"].(string); out,err := toolBrowserEval(ctx,e); return out,true,err
	case "sin_browser_wait": s,_ := args["selector"].(string); t,_ := args["timeout"].(string); out,err := toolBrowserWait(ctx,s,t); return out,true,err
