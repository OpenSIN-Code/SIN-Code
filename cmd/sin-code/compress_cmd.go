...
// Purpose: `sin-code compress` — deterministic + LLM-assisted compaction
// for SIN-Code's long-lived memory stores (lessons, instincts, summaries,
// AGENTS.md-shaped memory entries, and the on-disk AGENTS.md file).
//
// The CLI surface mirrors the caveman-compress pattern from
// JuliusBrussee/caveman: byte-preserving compaction of long-running memory
// with a strong byte-budget + a configurable retry-on-validation LLM pass.
//
// Subcommands:
//
//	sin-code compress plan   — Build + print a Plan (--dry-run on disk).
//	sin-code compress apply  — Build + Apply (writes snapshots + rewrites).
//	sin-code compress rollback <id>  — Restore from a snapshot file.
//
// Issue #172.
package main
