...
def test_deterministic_lint_repair_runs_once():
    ...
    plan1 = asyncio.run(factory.build_repair_plan(AgentTask(goal="g", repo_root="."), verdict))
    assert plan1 and plan1[0]["tool"] == "sin_bash"
    plan2 = asyncio.run(factory.build_repair_plan(AgentTask(goal="g", repo_root="."), verdict))
    assert plan2 == []
...
def test_llm_repair_parses_fenced_json():
    ...
    plan = asyncio.run(factory.build_repair_plan(AgentTask(goal="g", repo_root="."), verdict))
...
def test_policy_deny_beats_allow():
    ...
    allowed, reason = sandbox.decide("sin_bash", {"cmd": "git push --force origin main"})
...
def test_policy_default_deny():
    sandbox = PolicySandbox(
        default="deny",
        rules=[
            PolicyRule(action="allow", tool="sin_bash", arg="cmd", pattern="^pytest"),
        ],
    )
...
def test_policy_dry_run_logs_but_allows(tmp_path):
    ...
    async def scenario():
        ...
        router.register("sin_bash", echo)
        sandbox = PolicySandbox(
            rules=[PolicyRule(action="deny", tool="sin_bash", arg="cmd", pattern="rm -rf")],
            dry_run=True,
            audit_path=tmp_path / "audit.jsonl",
        )
        sandbox.wrap(router)
...
def test_depth_limit_blocks_fork_bombs():
    ctx = DelegationContext(
        max_depth=2, deadline_wall=time.monotonic() + 1000, min_budget_s=10, safety_margin_s=5
    )
...
def test_budget_shrinks_per_generation():
    ctx = DelegationContext(
        deadline_wall=time.monotonic() + 1000, safety_margin_s=10, min_budget_s=10
    )
...
def test_tool_health_flags_chronically_failing_tool(tmp_path):
    events = []
    for i in range(10):
        events.append({"event": "step_start", "step_id": f"e{i}", "tool": "sin_edit"})
        if i < 5:
...
def test_repair_hotspot_detects_lint_dominance(tmp_path):
    events = [{"event": "verdict", "ok": False, "kind": "fail_lint"} for _ in range(5)]
...
def test_budget_exhaustion_is_critical(tmp_path):
    events = [{"event": "budget_exhausted"} for _ in range(2)] + [
        {"event": "run_complete"} for _ in range(4)
    ]
    analyzer = TelemetryAnalyzer(_write_events(tmp_path, events))
    crits = [i for i in analyzer.analyze() if i.severity == "critical" and i.category == "stalls"]
    assert crits and "budget" in crits[0].finding


def test_prompt_rendering_includes_warns(tmp_path):
    events = [{"event": "delegate_done", "outcome": "success"} for _ in range(5)]
    analyzer = TelemetryAnalyzer(_write_events(tmp_path, events))
