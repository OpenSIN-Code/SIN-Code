...
def test_mcp_serve_handshake_lists_six_tools():
    ...
    try:
        ...
        resp = _send(proc, {"jsonrpc": "2.0", "id": 2, "method": "tools/list"})
        names = {t["name"] for t in resp["result"]["tools"]}
        assert {
            "sin_delegate",
            "sin_delegate_status",
            "sin_delegate_history",
            "sin_delegate_cancel",
            "sin_delegate_escalations",
            "sin_delegate_resolve",
        ...
        resp = _send(
            proc,
            {
                ...
                "params": {
                    ...
                    "arguments": {"plan_id": "no-such-plan-xyz"},
                },
            },
        )
        ...
        for i, plan_id in enumerate(("plan-a", "plan-b", "plan-c"), start=2):
            resp = _send(
                proc,
                {
                    ...
                    "id": i,
                    "method": "tools/call",
                    "params": {"name": "sin_delegate_cancel", "arguments": {"plan_id": plan_id}},
                },
            )
            payload = json.loads(resp["result"]["content"][0]["text"])
...
def test_mcp_serve_delegate_with_dry_run_plan():
    ...
    plan = {
        "goal": "echo hello",
        "tasks": [{"key": "k1", "title": "say hi", "instructions": "print hi", "backend": "echo"}],
    }
    proc = _start_server()
    ...
    try:
        ...
        resp = _send(
            proc,
            {
                ...
                "id": 2,
                "method": "tools/call",
                "params": {"name": "sin_delegate", "arguments": {"plan": plan, "dry_run": True}},
            },
        )
