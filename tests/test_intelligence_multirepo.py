...
def test_task_class_bucketing():
    assert task_class("high", ["a.py", "b.py"], ["diff", "tests"]) == "high:py:diff+tests"
...
def test_analytics_best_backend_returns_none_below_threshold(tmp_path):
    ...
    assert analytics.best_backend("low:any:diff", candidates=[("claude", "m1")]) is None
...
def test_governor_surplus_recycling(tmp_path):
    ...
    gov = BudgetGovernor(plan=plan, global_seconds=1200, priority={"A": 2, "B": 1})
...
def test_governor_deadline_pressure(tmp_path):
    ...
    plan = Plan(goal="g", tasks=(t,), repo=str(tmp_path))
    gov = BudgetGovernor(plan=plan, global_seconds=10_000, priority={"A": 1})
...
def test_resolve_requires_input_for_retry(tmp_path):
    broker = EscalationBroker(Ledger(tmp_path / "l.db"))
    esc = broker.raise_escalation("p1", "T1", "t", EscalationKind.GATE_FAILURE, "x", {})
    assert not broker.resolve("p1", esc.id, "retry")["ok"]
    assert broker.resolve("p1", esc.id, "retry", user_input="fix")["ok"]
...
def test_apply_resolutions_drop_yields_pending_or_skipped(tmp_path):
    ledger = Ledger(tmp_path / "l.db")
    plan = Plan(goal="g", repo=".", tasks=(Task(title="t", instructions="x", id="T1"),))
    broker = EscalationBroker(ledger)
    ledger.register_run(
        plan.id, "g", json.dumps({"goal": "g", "tasks": [{"id": "T1", "title": "t"}]})
    )
    esc = broker.raise_escalation(plan.id, "T1", "t", EscalationKind.GATE_FAILURE, "x", {})
...
def test_extract_contract_last_block_wins():
    out = (
        'bla <sin-contract>{"v": 1}</sin-contract> mehr <sin-contract>{"v": 2}</sin-contract> ende'
    )
...
def test_multirepo_plan_parsing_two_repos(tmp_path):
    ...
    mrp = multirepo_plan_from_dict(
        {
            "goal": "g",
            "repos": {"api": {"path": str(api)}, "web": {"path": str(web)}},
            "tasks": [
                {"key": "a", "repo": "api", "title": "endpoint", "instructions": "i"},
                {"key": "b", "repo": "web", "title": "client", "instructions": "i", "deps": ["a"]},
            ],
        }
    )
        ...
        multirepo_plan_from_dict(
            {
                ...
                "tasks": [{"key": "a", "repo": "ghost", "title": "t", "instructions": "i"}],
            }
        )
    ...
    mrp = multirepo_plan_from_dict(
        {
            ...
            "repos": {"api": {"path": str(api)}},
            "tasks": [
                {"title": "c", "instructions": "i", "deps": ["b"]},
                {"title": "b", "instructions": "i", "deps": ["a"]},
