...
def _make_task(title: str, deps=()) -> Task:
    return Task(
        ...
        agent=AgentSpec(
            backend="command",
            command=("sh", "-c", f"echo '{title}' > {title.replace(' ', '_')}.txt"),
        ),
...
def test_merge_saga_advances_both_repos_on_success(two_repos, tmp_path):
    ...
    mrp = _build_mrp(
        two_repos,
        [
            {"key": "a", "repo": "api", "title": a.title, "instructions": a.instructions},
            {
                "key": "b",
                "repo": "web",
                "title": b.title,
...
def test_merge_saga_rolls_back_all_repos_on_conflict(two_repos, tmp_path):
    ...
    subprocess.run(["git", "-C", str(api), "add", "-A"], capture_output=True, check=True)
    subprocess.run(
        [
            "git",
            "-C",
...
def test_ledger_records_phase2_rollback_event(two_repos, tmp_path):
    ...
    rollback = next(ev for ev in ledger.history(mrp.id) if ev["kind"] == "merge:phase2_rollback")
