...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for num_files in [100, 500, 1000]:
            ...
            dur = run_benchmark(
                f"discovery_{num_files}_py_files", target, run_discover, repo, "**/*.py"
            )
            if dur:
        ...
        for max_results in [10, 100, 1000]:
            ...
            dur = run_benchmark(
                f"max_results_early_stop_{max_results}",
                target,
                run_discover,
                repo,
                "**/*.py",
        ...
        dur = run_benchmark(
            "relevance_scoring_500", 5.0, run_discover, repo, "**/*.py", show_dependencies="true"
        )
        if dur:
        ...
        for ext_filter in ["py", "go", "js"]:
            ...
            dur = run_benchmark(
                f"extension_filter_{ext_filter}", target, run_discover, repo, f"**/*.{ext_filter}"
            )
            if dur:
