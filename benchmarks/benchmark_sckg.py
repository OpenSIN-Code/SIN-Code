...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for num_files in [100, 1000, 10000]:
            ...
            if stats:
                print(f"  Build {num_files}: {stats['duration']:.3f}s "
            ...
            for query_name, query, q_target in [
                ...
                q_time = run_benchmark(
                    f"{query_name}_{num_files}_files", q_target,
                    benchmark_query, num_files, query
                )
                if q_time:
            ...
            mem_mb = run_benchmark(
                f"memory_{num_files}_files", mem_target,
                benchmark_memory, num_files, repo
            )
            if mem_mb:
    ...
    print("=" * 60)
    for r in RESULTS:
        print(f"| {r['benchmark']:<25} | {r['result']:<10} | {r['target']:<10} | {r['status']:<6} |")
    print("=" * 60)
