...
def main():
    ...
    for _ in range(3):
        res = run_benchmark(
            "overhead_empty_command", 0.5,
            run_execute, "echo hello", timeout=30
        )
    if res:
    ...
    res = run_benchmark(
        "overhead_real_command", 1.0,
        run_execute, "ls -la /usr/bin", timeout=30
    )
    if res:
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".sh", delete=False) as f:
        ...
        lines.append('for i in $(seq 1 1000); do echo "padding line $i with some data to increase output size"; done')
        f.write("\n".join(lines))
        script_path = f.name
    ...
    try:
        res = run_benchmark(
            "secret_redaction_1mb_100_secrets", 3.0,
            run_execute, f"bash {script_path}", timeout=30
        )
        if res:
    ...
    timeout_val = 1
    res = run_benchmark(
        "timeout_kill_latency", 1.5,
        run_execute, "sleep 60", timeout=timeout_val
    )
    if res:
